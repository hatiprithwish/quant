...
export type TimeSection = "log" | "buckets" | "reports";

const SECTION_META: Record<TimeSection, { label: string; sub: string }> = {
  log:     { label: "CHRONICLE", sub: "time tracker" },
  buckets: { label: "BUCKETS",   sub: "manage buckets" },
  reports: { label: "REPORTS",   sub: "analytics" },
};

export default function TimePage({ section = "log" }: { section?: TimeSection }) {
  ...
  function openCreate() { setEditEntry(null); setShowLogModal(true); }
  function openEdit(entry: TimeActivity) { setEditEntry(entry); setShowLogModal(true); }
  function closeModal() { setShowLogModal(false); setEditEntry(null); }
  ...
  return (
    <>
      <style>{`
        ...
        .time-animate-in { animation: timeFadeIn 0.22s ease-out forwards; }
      `}</style>
          ...
          <div className="time-header">
            <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
              ...
              <div>
                ...
                <div style={{ fontFamily: G, fontSize: 8, letterSpacing: "0.12em", color: "rgba(6,182,212,0.4)", marginTop: 1 }}>
                  {meta.sub}
                </div>
              </div>
            </div>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
              {section === "log" && (
                <DateRangeDropdown accent={A} panelBg="#020c10" align="right" from={from} to={to} onChange={(f, t) => { setFrom(f); setTo(t); }} />
              )}
              {section === "log" && (
                <button
                  onClick={openCreate}
                  style={{
                    ...
                    padding: "5px 12px", background: A, border: "none", borderRadius: 4,
                    fontFamily: G, fontSize: 10, letterSpacing: "0.1em", fontWeight: 700,
                    color: "#000", cursor: "pointer",
                    boxShadow: "0 0 16px rgba(6,182,212,0.35)", transition: "all 0.15s",
                  }}
                  onMouseEnter={e => {
          ...
          <div className="time-body">
            {section === "log" && (
              <div className="time-animate-in" style={{ display: "flex", flexDirection: "column", gap: 16 }}>
                ...
                {error && (
                  <div style={{ border: "1px solid rgba(239,68,68,0.3)", background: "rgba(239,68,68,0.07)", borderRadius: 6, padding: "12px 16px", fontFamily: G, fontSize: 10, letterSpacing: "0.08em", color: "rgba(239,68,68,0.8)" }}>
                ...
                {data && data.days.length === 0 && !isLoading && (
                  <div style={{ border: "1px solid rgba(6,182,212,0.08)", borderRadius: 8, padding: "56px 0", textAlign: "center" }}>
                    ...
                    <div style={{ fontFamily: G, fontSize: 10, letterSpacing: "0.18em", color: "rgba(6,182,212,0.35)", fontWeight: 700 }}>NO ENTRIES</div>
                    <div style={{ fontFamily: G, fontSize: 9, color: "rgba(255,255,255,0.18)", marginTop: 8 }}>
                      Hit{" "}
                      <span style={{ color: A, cursor: "pointer", textDecoration: "underline", textDecorationColor: "rgba(6,182,212,0.3)" }} onClick={openCreate}>
                        + LOG TIME
                      </span>{" "}
            ...
            {section === "buckets" && (
              <div className="time-animate-in"><BucketsManager /></div>
            )}

            {section === "reports" && (
              <div className="time-animate-in"><TimeReports /></div>
            )}
          </div>
        </div>
      </div>
      ...
      {showLogModal && (
        <LogTimeModal entry={editEntry} buckets={activeBuckets} defaultDate={from} onClose={closeModal} />
      )}
    </>
  );
}
